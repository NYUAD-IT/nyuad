...
function remove_PrivilegedHelperTool()
{
    FNAME=$1
    [ -z "$FNAME" ] && fatal "remove_PrivilegedHelperTool(${FNAME}) must be passed a valid file name"
    # remove the PrivilegedHelperTools
    if [ -e /Library/PrivilegedHelperTools/${FNAME} ]; then
        echo "removing PrivilegedHelperTools: $FNAME"
        rm -f /Library/PrivilegedHelperTools/${FNAME}
        [ "$?" != "0" ] && ( echo "ERROR: failed to remove file: ${FNAME}  \n\t$!"; exit 1 )
        ((REMOVED++))
    fi
...
function remove_sharedtech()
{
    ...
    remove_LaunchDaemon com.ivanti.IvantiAgentTool.root.plist
    remove_PrivilegedHelperTool com.ivanti.IvantiAgentTool.root
...
function remove_baseagent()
{
	...
	remove_config_app "Ivanti Agent Settings.app"
